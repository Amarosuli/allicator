<script lang="ts">
	...
	async function deleteData(id: string) {
		try {
			await pb.collection('engine_models').delete(id);
...
<svelte:head>
	<title>Engine Models - Delete</title>
</svelte:head>
...
<div class="flex w-full flex-col items-center justify-center">
	...
	<div class="flex w-full flex-col items-center justify-center space-y-3 p-8 text-sm md:w-1/3">
		...
		<div class="flex w-full items-center justify-between">
			...
			<p class="w-full pr-2 text-right">{engineModel?.name}</p>
		</div>
		<Separator />
		<div class="flex w-full items-center justify-between">
			<p class="w-full pl-2 text-start font-bold">Description</p>
			<p class="w-full pr-2 text-right">{engineModel?.description}</p>
		</div>
		<Separator />
		<div class="flex w-full items-center justify-between">
			<p class="w-full pl-2 text-start font-bold">Family</p>
			<p class="w-full pr-2 text-right">{engineModel?.expand?.family_id.name}</p>
		</div>
		<Separator />
	</div>
