<script lang="ts">
	...
	async function deleteData(id: string) {
		try {
			await pb.collection('engine_modules').delete(id);
...
<svelte:head>
	<title>Engine Modules - Delete</title>
</svelte:head>
...
<div class="flex w-full flex-col items-center justify-center">
	...
	<div class="flex w-full flex-col items-center justify-center space-y-3 p-8 text-sm md:w-1/3">
		...
		<div class="flex w-full items-center justify-between">
			<p class="w-full pl-2 text-start font-bold">Name</p>
			<p class="w-full pr-2 text-right">{engineModule?.name}</p>
		</div>
		<Separator />
		<div class="flex w-full items-center justify-between">
			<p class="w-full pl-2 text-start font-bold">Description</p>
			<p class="w-full pr-2 text-right">{engineModule?.description}</p>
		</div>
		<Separator />
		<div class="flex w-full items-center justify-between">
			<p class="w-full pl-2 text-start font-bold">Parent Module</p>
			<p class="w-full pr-2 text-right">{engineModule?.expand ? `${engineModule.expand.parent_module.name} - ${engineModule.expand.parent_module.description}` : '-'}</p>
		</div>
		<Separator />
	</div>
