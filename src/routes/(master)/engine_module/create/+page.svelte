...
<svelte:head>
	<title>Engine Modules - Create</title>
</svelte:head>
			...
			<Field {form} name="parent_module" class="relative">
				<Control let:attrs>
					...
					<Select.Root
						...
						<Select.Trigger>
							<Select.Value class="w-full text-start" placeholder={selectedParent?.label || 'Select Parent'}></Select.Value>
						</Select.Trigger>
						...
						<Select.Content class="max-h-60 overflow-y-auto">
							<Select.Group>
								{#each engineModules as parent}
									<Select.Item value={parent.value} label={parent.label} class="text-ellipsis">{parent.label}</Select.Item>
								{/each}
							</Select.Group>
						</Select.Content>
						<Select.Input hidden {...attrs} bind:value={$formData.parent_module} type="text" />
					</Select.Root>
				</Control>
				<FieldErrors class="text-xs italic" />
			</Field>
