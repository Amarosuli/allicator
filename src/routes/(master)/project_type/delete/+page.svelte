<script lang="ts">
	...
	async function deleteData(id: string) {
		try {
			await pb.collection('project_type').delete(id);
...
<svelte:head>
	<title>Project Type - Delete</title>
</svelte:head>
...
<div class="flex w-full flex-col items-center justify-center">
	...
	<div class="flex w-full flex-col items-center justify-center space-y-3 p-8 text-sm md:w-1/3">
		...
		<div class="flex w-full items-center justify-between">
			<p class="w-full pl-2 text-start font-bold">Type Name</p>
			<p class="w-full pr-2 text-right">{projectType?.name}</p>
		</div>
		<Separator />
		<div class="flex w-full items-center justify-between">
			<p class="w-full pl-2 text-start font-bold">Type Code</p>
			<p class="w-full pr-2 text-right">{projectType?.code}</p>
		</div>
		<Separator />
		<div class="flex w-full items-center justify-between">
			<p class="w-full pl-2 text-start font-bold">Type Description</p>
			<p class="w-full pr-2 text-right">{projectType?.description}</p>
		</div>
		<Separator />
	</div>
