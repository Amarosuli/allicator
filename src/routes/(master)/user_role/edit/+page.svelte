<script lang="ts">
	...
	const form = superForm(data.form, {
		...
		onChange(event) {
			$formData.name = event.get('name').toUpperCase();
		}
	});
	const { form: formData, delayed, message, enhance } = form;
	const basePath = getFirstPath($page.url.pathname);
	$: data.userRole && formData.set(data.userRole);
</script>

<svelte:head>
	<title>User Role - Edit</title>
</svelte:head>

<div class="flex w-full flex-col items-center justify-center">
	<div class="text-2xl font-extrabold">Edit User Role</div>
		...
		<form class="flex w-full max-w-80 flex-col" method="post" use:enhance>
			<Field {form} name="name">
				<Control let:attrs>
					...
					<Input {...attrs} bind:value={$formData.name} type="text" placeholder="Role Name" />
				</Control>
			...
			<Field {form} name="description">
				<Control let:attrs>
					...
					<Input {...attrs} bind:value={$formData.description} type="text" placeholder="Role Description" />
				</Control>
				<FieldErrors class="text-xs italic" />
			</Field>
