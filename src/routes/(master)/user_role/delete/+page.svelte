<script lang="ts">
	...
	async function deleteData(id: string) {
		try {
			await pb.collection('user_roles').delete(id);
...
<div class="flex w-full flex-col items-center justify-center">
	...
	<div class="flex w-full flex-col items-center justify-center space-y-3 p-8 text-sm md:w-1/3">
		...
		<div class="flex w-full items-center justify-between">
			<p class="w-full pl-2 text-start font-bold">Role Name</p>
			<p class="w-full pr-2 text-right">{userRole?.name}</p>
		</div>
		<Separator />
		<div class="flex w-full items-center justify-between">
			<p class="w-full pl-2 text-start font-bold">Role Description</p>
			<p class="w-full pr-2 text-right">{userRole?.description}</p>
		</div>
		<Separator />
	</div>
