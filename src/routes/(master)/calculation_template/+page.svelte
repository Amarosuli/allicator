<script lang="ts">
	...
	const { nextPage, prevPage, getState } = createPageFile().init('calculation_tmp', { expand: 'module_id, engine_model_id' });
	...
	const columns = table.createColumns([
        ...
        table.column({
			...
			accessor: (item) => {
				if (item.expand && item.expand.module_id) return item.expand.module_id.name;
				return '-';
			},
			plugins: {
				filter: {
        ...
        table.column({
			header: 'Constanta',
			accessor: (item) => {
				if (item.constanta) return item.constanta;
				return '-';
			},
			plugins: { filter: { exclude: true } }
		}),
	...
	const hideableCols = ['title', 'description', 'Engine Model', 'Engine Module', 'formula', 'Constanta', 'subtask'];
</script>

<svelte:head>
	<title>Calculation Template</title>
</svelte:head>
...
<div class="relative mx-auto h-max w-full border p-4">
	...
	<div class="flex w-full items-center justify-between">
		<p class="w-full font-extrabold lg:text-xl">Calculation Template</p>
		{#if user}
