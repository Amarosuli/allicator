<script lang="ts">
	...
	async function deleteData(id: string) {
		try {
			await pb.collection('calculation_tmp').delete(id);
...
<svelte:head>
	<title>Calculation Template - Delete</title>
</svelte:head>
...
<div class="flex w-full flex-col items-center justify-center">
	...
	<div class="flex w-full flex-col items-center justify-center space-y-3 p-8 text-sm md:w-1/3">
		...
		<div class="flex w-full items-center justify-between">
			<p class="w-full pl-2 text-start font-bold">Title</p>
			<p class="w-full pr-2 text-right">{calculationTemplate?.title}</p>
		</div>
		<Separator />
		<div class="flex w-full items-center justify-between">
			<p class="w-full pl-2 text-start font-bold">Description</p>
			<p class="w-full pr-2 text-right">{calculationTemplate?.description}</p>
		</div>
        <Separator />
		<div class="flex w-full items-center justify-between">
			<p class="w-full pl-2 text-start font-bold">Engine Model</p>
			<p class="w-full pr-2 text-right">{calculationTemplate?.expand?.engine_model_id?.name}</p>
		</div>
        <Separator />
		<div class="flex w-full items-center justify-between">
			<p class="w-full pl-2 text-start font-bold">Engine Module</p>
			<p class="w-full pr-2 text-right">{calculationTemplate?.expand?.module_id?.name}</p>
		</div>
        <Separator />
		<div class="flex w-full items-center justify-between">
			<p class="w-full pl-2 text-start font-bold">Constanta</p>
			<p class="w-full pr-2 text-right">{calculationTemplate?.constanta}</p>
		</div>
        <Separator />
		<div class="flex w-full items-center justify-between">
			<p class="w-full pl-2 text-start font-bold">Subtask</p>
			<p class="w-full pr-2 text-right">{calculationTemplate?.subtask}</p>
		</div>
		<Separator />
	</div>
