<script lang="ts">
	...
	async function deleteData(id: string) {
		try {
			await pb.collection('project_list').delete(id);
...
<div class="flex w-full flex-col items-center justify-center">
	...
	<div class="flex w-full flex-col items-center justify-center space-y-3 p-8 text-sm md:w-1/3">
		...
		<div class="flex w-full items-center justify-between">
			<p class="w-full pl-2 text-start font-bold">Revision Number</p>
			<p class="w-full pr-2 text-right">{project?.revision_number}</p>
		</div>
		<Separator />
		<div class="flex w-full items-center justify-between">
			<p class="w-full pl-2 text-start font-bold">Engine Config</p>
			<p class="w-full pr-2 text-right">{project?.engine_config}</p>
		</div>
		<Separator />
		<div class="flex w-full items-center justify-between">
			<p class="w-full pl-2 text-start font-bold">Engine Model</p>
			<p class="w-full pr-2 text-right">{project?.expand?.engine_model_id.name}</p>
		</div>
		<Separator />
		<div class="flex w-full items-center justify-between">
			<p class="w-full pl-2 text-start font-bold">Engine Serial Number</p>
			<p class="w-full pr-2 text-right">{project?.expand?.engine_id.esn}</p>
		</div>
		<Separator />
		<div class="flex w-full items-center justify-between">
			<p class="w-full pl-2 text-start font-bold">Customer</p>
			<p class="w-full pr-2 text-right">{project?.expand?.customer.name}</p>
		</div>
		<Separator />
		<div class="flex w-full items-center justify-between">
			<p class="w-full pl-2 text-start font-bold">Project Type</p>
			<p class="w-full pr-2 text-right">{project?.expand?.project_type_id.name}</p>
		</div>
		<Separator />
		<div class="flex w-full items-center justify-between">
			<p class="w-full pl-2 text-start font-bold">Description</p>
			<p class="w-full pr-2 text-right">{project?.description}</p>
		</div>
		<Separator />
		<div class="flex w-full items-center justify-between">
			<p class="w-full pl-2 text-start font-bold">Status</p>
			<p class="w-full pr-2 text-right">{project?.status}</p>
		</div>
		<Separator />
	</div>
