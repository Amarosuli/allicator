<script lang="ts">
	...
	$: project &&
		formData.set({
			...
			started_at: new Date(project.started_at).toISOString().split('T')[0],
			finished_at: new Date(project.finished_at).toISOString().split('T')[0]
		});
...
<div class="flex w-full flex-col items-center justify-center">
	<div class="text-2xl font-extrabold">Edit Project</div>
			...
			<Field {form} name="project_type_id">
				<Control let:attrs>
					...
					<Select.Root
						...
						<Select.Trigger>
							<Select.Value class="w-full text-start" placeholder="Select Project Type" />
						</Select.Trigger>
						<Select.Content class="max-h-60 overflow-y-auto">
							<Select.Group>
